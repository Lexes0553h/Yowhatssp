...
const cohorts = [
  {
    id: 'founders',
    name: 'Tech Founders & VC Cohort',
    limit: 12,
    reserved: 9,
    perks: ['Private island buyout', 'Keynote fireside talks', 'Supercar convoy rally'],
    profiles: ['VC Partner, London', 'SaaS Founder, NYC', 'AI Engineer, SF']
  },
  {
    id: 'heritage',
    name: 'Multi-Generational Family Heritage',
    limit: 16,
    reserved: 10,
    perks: ['Private palace buyout', 'Michelin tasting banquet', 'Sovereign helicopter shuttle'],
    profiles: ['Enterprise Leader, Munich', 'Family Office Director, Zurich']
  },
  {
    id: 'alumni',
    name: 'Elite Executive Ivy Alumni',
    limit: 10,
    reserved: 8,
    perks: ['Swiss Alps chalet takeover', 'Closed-door leadership panel', 'Private ski guide team'],
    profiles: ['Partner, McKinsey NY', 'Managing Director, Goldman London']
  }
];

const destinations = [
  {
    title: 'Bali Villa Cohort',
...
export default function GroupTripsPage({ onClose, onBookNow }: PageProps) {
  const [selectedCohortId, setSelectedCohortId] = useState('founders');
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-[20%] -right-1/4 w-[600px] h-[600px] bg-rose-950/[0.12] rounded-full blur-[150px] animate-pulse" />
        <div className="absolute top-[65%] -left-1/4 w-[600px] h-[600px] bg-amber-950/[0.08] rounded-full blur-[140px]" />
      </div>
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-6 md:px-12 flex flex-col text-left">
          <div className="max-w-3xl space-y-4">
            <div className="flex items-center gap-2 text-neutral-400">
              <span className="text-[10px] font-mono font-bold tracking-[0.3em] uppercase text-rose-300">
                OUR SPECIALIZATION
              </span>
            </div>
            ...
            <p className="text-lg sm:text-xl md:text-2xl font-serif italic font-light text-neutral-300 tracking-wide">
              High-Society Connections & Collaborative Escapes.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="relative min-h-screen w-full flex flex-col justify-between bg-black text-left px-6 md:px-12 py-12 md:py-16">
        ...
        <div className="w-full max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-start mt-12 mb-8 relative z-10">
          {/* Left Text Narrative & Cohort Interactive Selector */}
          <div className="lg:col-span-7 space-y-8">
            <p className="text-neutral-300 text-base sm:text-lg lg:text-xl leading-relaxed font-light font-sans tracking-wide">
              Embark on magnificent cluster trips designed to form lifelong intellectual, social, and business connections. Tailored for high-net-worth families, enterprise circles, alumni networks, and global leaders wishing to share an extraordinary adventure with like-minded pioneers in pristine comfort.
            </p>

            {/* Interactive Selector */}
            <div className="pt-4 space-y-4">
              <span className="text-xs uppercase font-mono tracking-widest text-neutral-400 font-bold block">
                EXPLORE CURRENT ACTIVE COHORTS
              </span>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {cohorts.map((cohort) => (
                  <button
                    ...
                    className={`p-4 rounded-xl border text-left transition-all cursor-pointer bg-transparent outline-none ${selectedCohortId === cohort.id ? 'bg-rose-500/10 border-rose-400 text-white font-semibold shadow-lg shadow-rose-950/25' : 'border-white/5 text-neutral-400 hover:border-white/10'}`}
                  >
                    <span className="text-[10px] font-mono block text-rose-300 font-bold mb-1">COHORT</span>
                    <span className="text-xs leading-tight font-sans block font-semibold">{cohort.name}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="p-5 bg-white/[0.01] border border-white/5 rounded-[18px] space-y-4">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                  <div className="flex items-center gap-2 text-sm text-neutral-200 font-bold">
                    <Users className="w-4 h-4 text-rose-400" /> Member Capacity State
                  </div>
                  <div className="flex items-center gap-2 font-mono text-xs">
                    <span className="text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-full text-[10px]">
                      {activeCohort.limit - activeCohort.reserved} Seats Remaining
                    </span>
                    <span className="text-neutral-400">{activeCohort.reserved}/{activeCohort.limit} Reserved</span>
                  </div>
                </div>
                ...
                <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${(activeCohort.reserved / activeCohort.limit) * 100}%` }}
                  />
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                  <div className="space-y-2">
                    <span className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider block font-bold">COHORT PRIVILEGES</span>
                    <ul className="space-y-1.5 text-xs text-neutral-300 font-sans">
                      {activeCohort.perks.map((p, i) => (
                  ...
                  <div className="space-y-2">
                    <span className="text-[10px] font-mono text-neutral-400 uppercase tracking-wider block font-bold">ANONYMOUS PROFILES IN COHORT</span>
                    <ul className="space-y-1.5 text-[11px] text-neutral-400 font-mono italic">
                      {activeCohort.profiles.map((p, i) => (
                        <li key={i} className="flex items-center gap-2">
            ...
            <div className="bg-[#0c0c0e] border border-white/5 rounded-[24px] p-8 shadow-2xl">
              ...
              <ul className="space-y-4 text-left">
                {[
                  ...
                  'Celebratory Event Curations',
                  'Intellectual Companion Matchmaking',
                  'Private Cohort Group Guides'
                ].map((highlight, idx) => (
              ...
              {destinations.map((dest, idx) => (
                ...
                >
                  ...
                  <div className="absolute top-5 left-5">
                    <span className="px-3.5 py-1.5 rounded-full bg-rose-500/15 text-rose-300 border border-rose-500/30 text-[8px] sm:text-[9px] font-mono tracking-widest font-black uppercase flex items-center justify-center gap-1">
                      ★ {dest.tag}
                    </span>
                  </div>
        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-6 mt-12">
          <span className="text-[9px] font-mono tracking-[0.2em] text-neutral-500 uppercase font-medium">
            * ENTIRE PRIVATE ESTATE CHARTER WITH FULL-CREWED DOMESTIC TRANSIT MANAGEMENT.
          </span>

          <Magnetic strength={0.25}>
            <button
              onClick={onBookNow}
              className="w-full sm:w-auto bg-gradient-to-r from-rose-300 via-pink-300 to-amber-200 text-black text-xs font-bold tracking-[0.18em] uppercase px-8 py-4 rounded-full flex items-center justify-center gap-2.5 shadow-xl hover:shadow-rose-500/15 hover:brightness-105 active:scale-[0.98] transition-all outline-none border-none cursor-pointer"
            >
